...
def main() -> int:
    ...
    require(store, "activeHasDurableCommit", "no-op requires durable active commit proof")
    ...
    require(cfg_cli, "validateSemanticConfig", "config save rejects invalid authoritative config")
    forbid(cfg_cli, "ctx.config->sanitize();", "config save mutates invalid authoritative config")
    ...
    require(cal_cli, "trackerSerialCommitCalibrationCandidate", "shared calibration transaction")
    require(storage, "trackerApplyCalibrationCandidateToConfig", "field-wise runtime promotion")
    ...
    require(transaction, "void rollback(", "selector failure rollback")
    require(transaction, "commitPreparedPromotion", "prepared candidate commit")
    ...
    require(store, "prepareAuthoritativeCommit", "hardware transaction inactive-slot prepare")
    require(store, "commitPreparedAuthoritative", "hardware transaction selector commit")
    ...
    require(mag_runtime, "candidateConfig.validateSemanticConfig()", "mag-yaw semantic candidate verification")
