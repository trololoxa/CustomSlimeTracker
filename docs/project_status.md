...
- `setup calibration [axis <bodyX> <bodyY> <bodyZ>]` runs a blocking guided production calibration flow: rest/gyro, Wi-Fi heat warm-up, dedicated stationary gyro temperature capture until relative plateau, accel 6-position capture, magnetometer hard/soft collection, automatic mag axis inference, production tracking feature enable and one final transactional save. Failed/aborted setup calibration restores the previous RAM calibration/config and leaves the previous NVS calibration untouched.
