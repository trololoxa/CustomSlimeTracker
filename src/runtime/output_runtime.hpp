...
inline void outputPrintU64Dec(Stream& out, uint64_t v) {
    char buf[21];
    size_t i = sizeof(buf);
    buf[--i] = '\0';
    if (v == 0) {
...
inline void emitSerialStreamIfNeeded(TrackerSerialStreamState& streamState,
                                     Stream& out,
...
inline void maybePrintBootHeartbeat(Stream& out,
