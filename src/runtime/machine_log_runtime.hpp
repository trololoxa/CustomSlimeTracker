...
inline void machineLogPrintU64Dec(Stream& out, uint64_t v) {
    char buf[21];
    size_t i = sizeof(buf);
    buf[--i] = '\0';
    if (v == 0) {
...
inline const char* machineLogModeName(TrackerLogMode mode) {
    switch (mode) {
...
inline bool machineLogDue(TrackerSerialLogState& state, uint32_t nowUs) {
...
inline bool machineLogMagDue(TrackerSerialLogState& state, uint32_t nowUs) {
    if (!state.enabled()) return false;
    const uint32_t period = state.periodUs();
...
inline bool machineLogBiasDue(TrackerSerialLogState& state,
...
inline void machineLogResetCounters(MachineLogCounters& counters, uint32_t& lastBiasEmitUs) {
    counters = MachineLogCounters{};
    lastBiasEmitUs = 0;
}

inline void machineLogEmitHeader(Stream& out,
...
inline void machineLogEmitStateEvent(Stream& out,
...
inline void machineLogPrintSummary(Stream& out,
                                   const TrackerSerialLogState& state,
...
inline const char* machineLogBiasSource(const RuntimeGyroBiasEstimator& runtimeBias,
...
inline void machineLogEmitFrame(Stream& out,
                                TrackerSerialLogState& state,
                                MachineLogCounters& counters,
                                uint32_t& lastBiasEmitUs,
                                uint32_t biasPeriodUs,
...
inline void machineLogEmitMagFrame(Stream& out,
                                   TrackerSerialLogState& state,
                                   MachineLogCounters& counters,
