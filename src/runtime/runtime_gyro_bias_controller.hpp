...
inline bool runtimeBiasHasBaseGyroBiasModel(const ImuCalibration& imuCal,
                                            const GyroTempCompensator& gyroTempComp) {
    return gyroTempComp.valid() || imuCal.gyroBiasValid;
}

inline Vec3 runtimeBiasBaseGyroBiasRadS(const ImuCalibration& imuCal,
                                        const GyroTempCompensator& gyroTempComp,
                                        float tempC) {
...
inline Vec3 runtimeBiasCurrentGyroBiasRadS(const RuntimeGyroBiasEstimator& bias,
...
inline uint32_t runtimeBiasGyroBiasRuntimeFlags(const RuntimeGyroBiasEstimator& bias,
                                                const GyroTempCompensator& gyroTempComp,
                                                float tempC) {
...
inline void runtimeBiasApplyGyroTempQualityFlags(const GyroTempCompensator& gyroTempComp,
...
inline RuntimeBiasTempGate runtimeBiasTempGateFor(const RuntimeGyroBiasEstimator& bias,
                                                  const GyroTempCompensator& gyroTempComp,
                                                  float tempC) {
...
inline uint32_t runtimeBiasDecisionFlags(bool accepted,
                                         bool badTiming,
                                         bool saturated,
                                         bool calibrationBad,
                                         bool tempBad,
...
inline void emitRuntimeBiasUpdateLog(const RuntimeGyroBiasUpdateDeps& deps,
                                     uint64_t tUs,
                                     float tempC,
...
inline Vec3 clampRuntimeTrimDps(const RuntimeGyroBiasEstimator& bias, const Vec3& trimDps) {
...
inline void runtimeBiasUpdateEstimator(const RuntimeGyroBiasUpdateDeps& deps,
...
inline void runtimeBiasPrintStatus(Stream& out,
...
inline void runtimeBiasSetEnabled(RuntimeGyroBiasEstimator& bias, bool enabled) {
    bias.enabled = enabled;
    bias.consecutiveStationaryWindows = 0;
    bias.resetWindow();
}

inline void runtimeBiasReset(RuntimeGyroBiasEstimator& bias) {
