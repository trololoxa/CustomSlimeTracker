...
void captureMotionSlimeBaseline(const SlimeVROutputRuntime* slime) {
    ...
    g_motionSlimeBaseline.accelerationSent = s.accelerationSent;
    g_motionSlimeBaseline.compactMotionSent = s.compactMotionSent;
    g_motionSlimeBaseline.compactMotionSendFailures = s.compactMotionSendFailures;
    g_motionSlimeBaseline.bundledMotionSent = s.bundledMotionSent;
    g_motionSlimeBaseline.bundledMotionSendFailures = s.bundledMotionSendFailures;
    g_motionSlimeBaseline.accelerationRateLimited = s.accelerationRateLimited;
    g_motionSlimeBaseline.udpTransportRebindSuccesses = s.udpTransportRebindSuccesses;
    g_motionSlimeBaseline.udpFullReopenEscalations = s.udpFullReopenEscalations;
    g_motionSlimeBaseline.txBackoffDrops = s.txBackoffDrops;
    g_motionSlimeBaseline.txPressureFailures = s.txPressureFailures;
    g_motionSlimeBaseline.txPressureEpisodeCount = s.txPressureEpisodeCount;
    g_motionSlimeBaseline.physicalDatagramsSent = s.physicalDatagramsSent;
    g_motionSlimeBaseline.motionDatagramsSent = s.motionDatagramsSent;
    g_motionSlimeBaseline.separateRotationDatagramsSent = s.separateRotationDatagramsSent;
    g_motionSlimeBaseline.separateAccelerationDatagramsSent = s.separateAccelerationDatagramsSent;
    g_motionSlimeBaseline.accelerationSuppressedDuringNegotiation = s.accelerationSuppressedDuringNegotiation;
    g_motionSlimeBaseline.txFailureWindowTrips = s.txFailureWindowTrips;
    g_motionSlimeBaseline.accelerationSkippedInvalid = s.accelerationSkippedInvalid;
    g_motionSlimeBaseline.accelerationSkippedConfiguration = s.accelerationSkippedConfiguration;
    g_motionSlimeBaseline.accelerationSkippedComponentMissing = s.accelerationSkippedComponentMissing;
    g_motionSlimeBaseline.accelerationSkippedPairDegraded = s.accelerationSkippedPairDegraded;
    g_motionSlimeBaseline.accelerationSkippedSaturated = s.accelerationSkippedSaturated;
    g_motionSlimeBaseline.accelerationSkippedNonFinite = s.accelerationSkippedNonFinite;
    g_motionSlimeBaseline.accelerationSkippedOther = s.accelerationSkippedOther;
    g_motionSlimeBaseline.accelerationSendFailures = s.accelerationSendFailures;
...
void printFifoMotion(Stream& out, const Lsm6dsvFifoReader* fifo) {
    ...
    out.print("fifo_complete_pairs_produced="); out.println(s.completePairsProduced);
    out.print("fifo_gyro_only_samples_produced="); out.println(s.gyroOnlySamplesProduced);
    out.print("fifo_gyro_pending_replaced="); out.println(s.gyroPendingReplaced);
    out.print("fifo_accel_pending_replaced="); out.println(s.accelPendingReplaced);
    out.print("fifo_pair_counter_mismatches="); out.println(s.pairCounterMismatches);
    out.print("fifo_pair_counter_offset_locks="); out.println(s.pairCounterOffsetLocks);
    out.print("fifo_pair_counter_offset_relocks="); out.println(s.pairCounterOffsetRelocks);
...
void printSlimeMotion(Stream& out, const SlimeVROutputRuntime* slime, uint32_t windowMs) {
    ...
    out.print("slime_motion_packet_mode="); out.println(slimevrMotionPacketModeName(s.motionPacketMode));
    out.print("slime_compact_motion_sent_delta="); out.println(hasBaseline ? deltaU32(s.compactMotionSent, g_motionSlimeBaseline.compactMotionSent) : s.compactMotionSent);
    out.print("slime_compact_motion_send_failures_delta="); out.println(hasBaseline ? deltaU32(s.compactMotionSendFailures, g_motionSlimeBaseline.compactMotionSendFailures) : s.compactMotionSendFailures);
    out.print("slime_bundled_motion_sent_delta="); out.println(hasBaseline ? deltaU32(s.bundledMotionSent, g_motionSlimeBaseline.bundledMotionSent) : s.bundledMotionSent);
    out.print("slime_bundled_motion_send_failures_delta="); out.println(hasBaseline ? deltaU32(s.bundledMotionSendFailures, g_motionSlimeBaseline.bundledMotionSendFailures) : s.bundledMotionSendFailures);
    out.print("slime_acceleration_rate_limited_delta="); out.println(hasBaseline ? deltaU32(s.accelerationRateLimited, g_motionSlimeBaseline.accelerationRateLimited) : s.accelerationRateLimited);
    out.print("slime_udp_transport_rebind_successes_delta="); out.println(hasBaseline ? deltaU32(s.udpTransportRebindSuccesses, g_motionSlimeBaseline.udpTransportRebindSuccesses) : s.udpTransportRebindSuccesses);
    out.print("slime_udp_full_reopen_escalations_delta="); out.println(hasBaseline ? deltaU32(s.udpFullReopenEscalations, g_motionSlimeBaseline.udpFullReopenEscalations) : s.udpFullReopenEscalations);
    out.print("slime_tx_backoff_drops_delta="); out.println(hasBaseline ? deltaU32(s.txBackoffDrops, g_motionSlimeBaseline.txBackoffDrops) : s.txBackoffDrops);
    out.print("slime_tx_pressure_failures_delta="); out.println(hasBaseline ? deltaU32(s.txPressureFailures, g_motionSlimeBaseline.txPressureFailures) : s.txPressureFailures);
    ...
    out.print("slime_tx_pressure_episode_count_delta="); out.println(hasBaseline ? deltaU32(s.txPressureEpisodeCount, g_motionSlimeBaseline.txPressureEpisodeCount) : s.txPressureEpisodeCount);
    ...
    out.print("slime_physical_datagrams_sent_delta="); out.println(hasBaseline ? deltaU32(s.physicalDatagramsSent, g_motionSlimeBaseline.physicalDatagramsSent) : s.physicalDatagramsSent);
    out.print("slime_motion_datagrams_sent_delta="); out.println(hasBaseline ? deltaU32(s.motionDatagramsSent, g_motionSlimeBaseline.motionDatagramsSent) : s.motionDatagramsSent);
    out.print("slime_separate_rotation_datagrams_sent_delta="); out.println(hasBaseline ? deltaU32(s.separateRotationDatagramsSent, g_motionSlimeBaseline.separateRotationDatagramsSent) : s.separateRotationDatagramsSent);
    out.print("slime_separate_acceleration_datagrams_sent_delta="); out.println(hasBaseline ? deltaU32(s.separateAccelerationDatagramsSent, g_motionSlimeBaseline.separateAccelerationDatagramsSent) : s.separateAccelerationDatagramsSent);
    out.print("slime_acceleration_suppressed_during_negotiation_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSuppressedDuringNegotiation, g_motionSlimeBaseline.accelerationSuppressedDuringNegotiation) : s.accelerationSuppressedDuringNegotiation);
    out.print("slime_rotation_phase_offset_ms="); out.println(s.rotationPhaseOffsetMs);
    out.print("slime_tx_failure_window_trips_delta="); out.println(hasBaseline ? deltaU32(s.txFailureWindowTrips, g_motionSlimeBaseline.txFailureWindowTrips) : s.txFailureWindowTrips);
    out.print("slime_acceleration_sent_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSent, g_motionSlimeBaseline.accelerationSent) : s.accelerationSent);
    out.print("slime_acceleration_skipped_invalid_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedInvalid, g_motionSlimeBaseline.accelerationSkippedInvalid) : s.accelerationSkippedInvalid);
    out.print("slime_acceleration_skipped_configuration_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedConfiguration, g_motionSlimeBaseline.accelerationSkippedConfiguration) : s.accelerationSkippedConfiguration);
    out.print("slime_acceleration_skipped_component_missing_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedComponentMissing, g_motionSlimeBaseline.accelerationSkippedComponentMissing) : s.accelerationSkippedComponentMissing);
    out.print("slime_acceleration_skipped_pair_degraded_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedPairDegraded, g_motionSlimeBaseline.accelerationSkippedPairDegraded) : s.accelerationSkippedPairDegraded);
    out.print("slime_acceleration_skipped_saturated_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedSaturated, g_motionSlimeBaseline.accelerationSkippedSaturated) : s.accelerationSkippedSaturated);
    out.print("slime_acceleration_skipped_nonfinite_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedNonFinite, g_motionSlimeBaseline.accelerationSkippedNonFinite) : s.accelerationSkippedNonFinite);
    out.print("slime_acceleration_skipped_other_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSkippedOther, g_motionSlimeBaseline.accelerationSkippedOther) : s.accelerationSkippedOther);
    out.print("slime_acceleration_send_failures_delta="); out.println(hasBaseline ? deltaU32(s.accelerationSendFailures, g_motionSlimeBaseline.accelerationSendFailures) : s.accelerationSendFailures);
    ...
    out.print("slime_last_rotation_software_age_us="); out.println(s.lastRotationSoftwareAgeUs);
