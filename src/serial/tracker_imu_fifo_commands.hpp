...
inline const char* trackerSerialOdrName(Lsm6dsv::Odr odr) {
    switch (odr) {
...
inline bool trackerSerialParseRuntimeOdr(const char* token, Lsm6dsv::Odr& outOdr) {
...
inline void trackerSerialPrintFifoStats(Stream& out, const Lsm6dsvFifoReader::DrainStats& fs) {
...
inline void trackerSerialPrintQualityStats(Stream& out, const ImuQualityCounters& qc) {
...
inline void trackerSerialPrintImuRuntimeStatus(TrackerSerialCommandContext& ctx, Stream& out) {
...
inline bool trackerSerialLiveReconfigureImuFifo(TrackerSerialCommandContext& ctx, Stream& out) {
...
inline bool trackerSerialSaveConfigIfRequested(TrackerSerialCommandContext& ctx, Stream& out, bool save) {
    if (!save) return true;
...
inline void trackerSerialDispatchImuCommand(TrackerSerialCommandContext& ctx, int argc, char** argv) {
...
inline void trackerSerialDispatchFifoCommand(TrackerSerialCommandContext& ctx, int argc, char** argv) {
    Stream& out = ctx.io ? *ctx.io : Serial;
    if (!ctx.fifo) {
...
inline void trackerSerialDispatchQualityCommand(TrackerSerialCommandContext& ctx, int argc, char** argv) {
    Stream& out = ctx.io ? *ctx.io : Serial;
    if (!ctx.quality) {
