...
inline Stream& stream(TrackerSerialCommandContext& ctx) {
...
inline bool magSaveConfigIfRequested(TrackerSerialCommandContext& ctx, bool saveRequested) {
...
inline void rearmMagIfNeeded(TrackerSerialCommandContext& ctx) {
...
inline char magAxisLower(char c) {
        if (c >= 'A' && c <= 'Z') return static_cast<char>(c - 'A' + 'a');
        return c;
    }

inline bool parseMagAxisToken(const char* token, uint8_t& axis, float& sign) {
...
inline const char* magAxisNameFromIndex(uint8_t axis) {
        switch (axis) {
            case 0: return "x";
            case 1: return "y";
            case 2: return "z";
...
inline void printMagAxisToken(Stream& out, const Mat3& m, uint8_t row) {
...
inline bool makeMagAxisMatrixFromTokens(const char* bodyXToken,
...
inline bool saveConfigIfRequested(TrackerSerialCommandContext& ctx, bool saveRequested) {
        if (!saveRequested) return true;
        if (!ctx.config || !ctx.configStore) return false;
        ctx.config->updateCrc();
        return ctx.configStore->save(*ctx.config);
...
inline void printMagAxisMatrix(Stream& out, const TrackerConfig& config) {
    ...
    inline void trackerSerialDispatchMagCommand(TrackerSerialCommandContext& ctx, int argc, char** argv) {
