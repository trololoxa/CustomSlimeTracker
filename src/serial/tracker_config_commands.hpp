...
inline Stream& trackerSerialConfigStream(TrackerSerialCommandContext& ctx) {
    return ctx.io ? *ctx.io : Serial;
}

inline bool trackerSerialConfigIs(const char* a, const char* b) {
    return tracker_serial_detail::eqIgnoreCase(a, b);
}

inline void trackerSerialApplyConfigToRuntime(TrackerSerialCommandContext& ctx) {
...
inline void trackerSerialCaptureRuntimeToConfig(TrackerSerialCommandContext& ctx) {
    if (!ctx.config) return;
...
inline void trackerSerialPrintConfigNvsInfo(Stream& out, TrackerConfigStore& store) {
...
inline void trackerSerialDispatchConfigCommand(TrackerSerialCommandContext& ctx, int argc, char** argv) {
